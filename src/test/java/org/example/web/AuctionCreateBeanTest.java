...
class AuctionCreateBeanTest {
    ...
    @Test
    void save_WithInvalidDates_ShouldNotCreateAuction() {
        // Arrange
        when(userSession.getUser()).thenReturn(testUser);
        auctionCreateBean.setStartTime(TEST_END_TIME);
        auctionCreateBean.setEndTime(TEST_START_TIME);
        ...
        try (MockedStatic<JSFUtil> mockedJSFUtil = mockStatic(JSFUtil.class)) {
            ...
            mockedJSFUtil.verify(() -> JSFUtil.addErrorMessage("Start time must be before end time"));
        }
    }

    @Test
    void save_WithNegativePrice_ShouldNotCreateAuction() {
        // Arrange
        when(userSession.getUser()).thenReturn(testUser);
        ...
        try (MockedStatic<JSFUtil> mockedJSFUtil = mockStatic(JSFUtil.class)) {
            ...
            // Assert
            verify(auctionService, never()).saveAuction(any());
            mockedJSFUtil.verify(() -> JSFUtil.addErrorMessage("Start price must be positive"));
        }
    }

    @Test
    void save_WithZeroPrice_ShouldNotCreateAuction() {
        // Arrange
        when(userSession.getUser()).thenReturn(testUser);
        auctionCreateBean.setStartPrice(BigDecimal.ZERO);
    ...
    @Test
    void save_WithNullPrice_ShouldNotCreateAuction() {
        // Arrange
        when(userSession.getUser()).thenReturn(testUser);
        auctionCreateBean.setStartPrice(null);
    ...
    @Test
    void validateInput_WithNullNameShouldInformUser() {
        // Arrange
        User testUser = new User();
        testUser.setUserId(1L);
        when(userSession.getUser()).thenReturn(testUser); // Mock logged in user

        auctionCreateBean.setName(null);
        auctionCreateBean.setStartTime(LocalDateTime.now());
        auctionCreateBean.setEndTime(LocalDateTime.now().plusDays(1));
        auctionCreateBean.setStartPrice(new BigDecimal("100.00"));
        ...
        try (MockedStatic<JSFUtil> mockedJSFUtil = mockStatic(JSFUtil.class)) {
            ...
            // Assert
            verify(auctionService, never()).saveAuction(any());
            mockedJSFUtil.verify(() -> JSFUtil.addErrorMessage("Name is required"));
        }
    }

    @Test
    void validateInput_WithEmptyTrimShouldInformUser() {
        // Arrange
        User testUser = new User();
        testUser.setUserId(1L);
        when(userSession.getUser()).thenReturn(testUser); // Mock logged in user

        auctionCreateBean.setName("");
        auctionCreateBean.setStartTime(LocalDateTime.now());
        auctionCreateBean.setEndTime(LocalDateTime.now().plusDays(1));
        auctionCreateBean.setStartPrice(new BigDecimal("100.00"));
    ...
    @Test
    void validateInput_WithNullStartTimeShouldInformUser() {
        // Arrange
        User testUser = new User();
        testUser.setUserId(1L);
        when(userSession.getUser()).thenReturn(testUser); // Mock logged in user

        auctionCreateBean.setName("name");
        auctionCreateBean.setStartTime(null);
        auctionCreateBean.setEndTime(LocalDateTime.now().plusDays(1));
        auctionCreateBean.setStartPrice(new BigDecimal("100.00"));
        ...
        try (MockedStatic<JSFUtil> mockedJSFUtil = mockStatic(JSFUtil.class)) {
            ...
            // Assert
            verify(auctionService, never()).saveAuction(any());
            mockedJSFUtil.verify(() -> JSFUtil.addErrorMessage("Start and end times are required"));
        }
    }

    @Test
    void validateInput_WithNullEndTimeShouldInformUser() {
        // Arrange
        User testUser = new User();
        testUser.setUserId(1L);
        when(userSession.getUser()).thenReturn(testUser); // Mock logged in user
        ...
        auctionCreateBean.setName("name");
        auctionCreateBean.setStartTime(LocalDateTime.now());
        auctionCreateBean.setEndTime(null);
        auctionCreateBean.setStartPrice(new BigDecimal("100.00"));
